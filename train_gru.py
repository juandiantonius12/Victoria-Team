"""
Train the GRU component of the model.
...
of recent history, with PM10 as the primary pollutant signal in that
sequence. It predicts the raw PM2_5_next_hour level, using a learned
per-station embedding so the network can pick up on station-specific
baseline behaviour.
...
# Raw per-hour signals the sequence model reads directly.
SEQUENCE_COLUMNS = [
    "PM10", "SO2", "NO2", "CO", "O3", "TEMP", "PRES", "DEWP", "RAIN", "WSPM",
    "wind_u", "wind_v", "hour_sin", "hour_cos", "city_mean_pm10",
]
...
def main():
    ...
    grid["city_mean_pm10"] = grid.groupby(level=1).PM10.transform("mean")

    seq_data = grid[SEQUENCE_COLUMNS].copy()
    missing_flags = seq_data[["PM10", "NO2", "CO", "O3"]].isna().astype(np.float32).add_suffix("_missing")
    ...
    target_scale = float(train[TARGET].std())
    ...
    train_targets = torch.tensor(TR[TARGET].values / target_scale, dtype=torch.float32).to(device)
    ...
    pred = np.clip(torch.cat(outputs).cpu().numpy() * target_scale, 0, None)
